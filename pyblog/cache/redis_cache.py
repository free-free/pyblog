...
class RedisCacheClient(object):
    ...
    def delete(self, key, key_prefix):
        ...
        if key_type == "string":
            ...
            result = self._connection.hdel(
                key, *self._connection.hkeys(key_prefix + key))
        elif key_type == "list":
            length = self._connection.llen(key_prefix + key)
            if length:
                result = self._connection.ltrim(
                    key_prefix + key, length + 1, length + 1)
        else:
            result = None
...
class AsyncRedisCache(CacheAbstractDriver):
    ...
    def __init__(self, host, port, cache_db, *args, **kwargs):
        self.__client = AsyncRedisCacheClient(
            host, port, cache_db, *args, **kwargs)
