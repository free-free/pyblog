...
class DB(dict):
    ...
    def getsql(self):
        ...
        if 'table' not in self or not self['table']:
            Log.error('table name can\'t be empty')
            return False
        fullsql = ' select %s from `%s` %s' % (self['fields'], self['table'], self[
                                               'join'] if 'join' in self else '')
        self['join'] = ''
        self['fields'] = ''
        self['table'] = ''
        ...
        if not sql == None:
            ...
            if isinstance(sql, dict):
                ...
                for k, v in sql.items():
                    setitems.append(' %s=%s' % (
                        se(k), '"' + v + '"' if isinstance(v, str) else str(v)))
    ...
    def insert(self, sql=None, args=None):
        ...
        if not sql == None:
            ...
            if isinstance(sql, str):
                return self.__fillparam(sql, args)
            # for query builder,it's recommend to insert a large number of data
            # set
            if isinstance(sql, list) and isinstance(sql[0], dict):
                ...
                for insert_item in sql:
                    columns = ','.join(
                        map(lambda item: '`' + item + '`', insert_item.keys()))
                    columns_value = ','.join(map(
                        lambda item: '"' + item + '"' if isinstance(item, str) else str(item), insert_item.values()))
                    fullsql.append('insert into `%s`(%s) values(%s)' %
                                   (self['table'], columns, columns_value))
                self['table'] = ''
                return ';'.join(fullsql)
    ...
    def where(self, column, operator, value):
        ...
        if 'where' in self and not self['where'] == '':
            self['where'] = self['where'] + ' and `%s`%s%s' % (column.strip(), str(
                operator).strip(), '"' + value + '"' if isinstance(value, str) else str(value))
        else:
            self['where'] = ' where `%s`%s%s ' % (str(column).strip(), str(operator).strip(
    ...
    def orwhere(self, column, operator, value):
        ...
        if 'where' in self and not self['where'] == '':
            if str(operator).strip() not in standard_operator:
                Log.error('operator is not correct in %s' % __file__)
                return False
            self['where'] = self['where'] + ' or `%s`%s%s' % (column.strip(), str(
                operator).strip(), '"' + value + '"' if isinstance(value, str) else str(value))
            return self
        return self
    ...
    def wherein(self, column, valtuple):
        ...
        if 'where' in self and not self['where'] == '':
            self['where'] = self['where'] + \
                ' and `%s` in %s' % (column.strip(), valtuple)
        else:
    ...
    def wherenotin(self, column, valtuple):
        ...
        if 'where' in self and not self['where'] == '':
            self['where'] = self['where'] + \
                ' and `%s` not in %s' % (column.strip(), valtuple)
        else:
            self['where'] = ' where `%s` not in %s' % (
                column.strip(), valtuple)
        return self
    ...
    def wherebetween(self, column, valrange):
        if not isinstance(valrange, tuple):
            ...
            return False
        if 'where' in self and not self['where'] == '':
            self['where'] = self['where'] + ' and `%s`>=%s and `%s`<%s' % (
                column.strip(), valrange[0], column.strip(), valrange[-1])
        else:
            self['where'] = ' where `%s`>=%s and `%s`<%s' % (
                column.strip(), valrange[0], column.strip(), valrange[-1])
        return self
    ...
    def wherenotbetween(self, column, valrange):
        if not isinstance(valrange, tuple):
            ...
            return False
        if 'where' in self and not self['where'] == '':
            self['where'] = self['where'] + ' and `%s`>%s or `%s`<%s' % (
                column.strip(), valrange[-1], column.strip(), valrange[0])
        else:
            self['where'] = 'where `%s`>%s or `%s`<%s' % (
                column.strip(), valrange[-1], column.strip(), valrange[0])
        return self
    ...
    def join(self, innertable, column1, operator, column2):
        ...
        if len(column1) == 2 and len(column1) == 2:
            self['join'] = ' inner join `%s` on  %s.`%s` = %s.`%s` ' % (
                innertable, column1[0], column1[1], column2[0], column2[1])
        elif len(column1) == 1 and len(column2) == 2:
            self['join'] = ' inner join `%s` on  `%s`=`%s` ' % (
                innertable, column1[0], column2[0])
        else:
            return False
        return self
    ...
    def leftjoin(self, innertable, column1, operator, column2):
        ...
        column2 = re.split('\.', column2)
        if len(column1) == 2 and len(column1) == 2:
            self['join'] = ' left  join `%s` on %s.`%s` =%s.`%s` ' % (
                innertable, column1[0], column1[1], column2[0], column2[1])
        elif len(column2) == 1 and len(column2) == 1:
            self['join'] = ' left join `%s` on `%s` = `%s` ' % (
                innertable, column1[0], column2[0])
        else:
            return False
        return self
    ...
    def having(self, column, operator, value):
        ...
        if str(operator).strip() not in standard_operator:
            Log.error('operator is not correct in %s' % __file__)
            return False
        self['having'] = ' having `%s`%s%s' % (str(column).strip(), str(operator).strip(
        ), '"' + value.strip() + '"' if isinstance(value, str) else str(value))
        return self
