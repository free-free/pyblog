...
class ConfigLoader(object):
    ...
    def _get_specific_driver_all_config_item(self, driver_name):
        if driver_name.lower() not in self._all_drivers_name:
            raise AttributeError(" %s config has no driver '%s'" % (
                self._config_name, driver_name.lower()))
    ...
    def _get_specific_driver_config_item(self, item_name, driver_name):
        ...
        if item_name.lower() not in config.keys():
            raise AttributeError(" %s config driver '%s' has no '%s' config item" % (
                self._config_name, driver_name, item_name))
    ...
    def __getattr__(self, key):
        ...
        if not self._specific_driver_name:
            ...
            if key.lower() == 'all':
                config = self._get_specific_driver_all_config_item(
                    self._specific_driver_name)
            elif key.lower() == 'driver_name':
                config = self._specific_driver_name
            else:
                config = self._get_specific_driver_config_item(
                    key, self._specific_driver_name)
            self._specific_driver_name = None
            return config
    ...
    def __getattr__(self, key):
        ...
        if not self._specific_connection:
            ...
            if key.lower() == 'connection_name':
                ...
                self._specific_connection = None
                return conn_name
            item = self._get_specific_connection_config_item(
                key, self._specific_connection)
            self._specific_connection = None
            return item
...
class SessionConfigLoader(object):
    ...
    def _get_specific_driver_config_item(self, item_name, driver_name):
        if driver_name.lower() not in self._all_drivers:
            raise AttributeError(
                "session config has no driver '%s'" % (driver_name))
        if item_name in self._config['drivers'][driver_name]:
            return self._config['drivers'][driver_name].get(item_name)
        else:
            raise AttributeError(
                "session driver %s has no such item '%s'" % (driver_name, item_name))

    def _get_specific_driver_all_config_item(self, driver_name):
        if driver_name.lower() not in self._all_drivers:
            raise AttributeError(
                "session config has no driver '%s'" % (driver_name.lower()))
    ...
    def __getattr__(self, key):
        ...
        if not self._specific_driver:
            ...
            if key.lower() == 'driver_name':
                ...
                self._specific_driver = None
                return driver_name
            item = self._get_specific_driver_config_item(
                key, self._specific_driver)
            self._specific_driver = None
            return item
...
class AuthConfigLoader(object):
    ...
    def __getattr__(self, key):
        if key not in self._config:
            raise AttributeError(
                "authentication config has no such item '%s'" % (key))
...
class StorageConfigLoader(object):
    ...
    def __init__(self):
        ...
        self.__default_disk = self.__config[
            'disks'].get(self.__config.get('default'))
    ...
    def _get_disk_config(self, item, disk):
        if disk.lower() not in self.__all_disks:
            raise AttributeError(
                "storage config has no such driver '%s'" % disk.lower())
        if item.lower() not in self.__all_disks[disk.lower()]:
            raise AttributeError("storage config driver '%s' has no such config item '%s'" % (
                disk.lower(), item.lower()))
...
class QueueConfigLoader(object):
    ...
    def _get_specific_driver_all_config_item(self, driver_name):
        if driver_name.lower() not in self._all_drivers_name:
            raise AttributeError(
                " queue config has no  '%s' driver " % (driver_name.lower()))
        return self._config.get('drivers').get(driver_name.lower())

    def _get_specific_driver_config_item(self, item_name, driver_name):
        config = self._get_specific_driver_all_config_item(driver_name)
        if item_name.lower() not in config:
            raise AttributeError("queue driver '%s' has no  '%s' config item" % (
                driver_name.lower(), item_name.lower()))
        return config.get(item_name.lower())
...
class CacheConfigLoader(object):
    ...
    def __init__(self):
        ...
        self._default_driver_name = self._config.get("default")
        self._default_driver = self._config.get(
            "drivers").get(self._default_driver_name)
    ...
    def _get_driver_config(self, item, driver_name):
        ...
        if item not in config:
            raise ConfigError(
                "cache driver '%s' has no config item '%s'" % (driver_name, item))
