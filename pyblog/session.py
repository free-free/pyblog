...
class AsyncRedisSession(AsyncAbstractSession):
    ...
    def __init__(self, session_id=None, config=None, loop=None):
        ...
        if not config:
            ...
            assert isinstance(
                config, dict), "redis connection config must dict type"
    ...
    def session(self, loop=None):
        ...
        if not self[self._session_id]:
            ...
            expire_delta_time = self[self._session_id].get(
                b"__expire_delta_time", b'').decode("utf-8")
...
class FileSession(AbstractSession):
    ...
    def __init__(self, session_id=None, config=None):
        ...
        if os.path.exists(self._session_file):
            ...
            expire_time = self._data[self._session_id].get(
                '__expire_delta_time', None)
            last_active_time = self._data[
                self._session_id].get('__last_active_time')
            if expire_time:
    ...
    def save(self, expire=None):
        ...
        if expire:
            ...
            if int(self._session_expire) != 0:
                self._data[self._session_id][
                    '__expire_delta_time'] = self._session_expire
    ...
    def renew(self, session_id=None):
        ...
        if os.path.exists(self._session_file):
            ...
            expire_time = self._data[self._session_id].get(
                '__expire_delta_time', None)
            last_active_time = self._data[
                self._session_id].get('__last_expire_time')
            if expire_time:
                if (int(time.time) - last_active_time) > expire_time:
                    os.remove(self._session_file)
                    self._data[self._session_id] = {}
...
class MongoSession(AbstractSession):
    ...
    def __init__(self, session_id=None, config=None):
        ...
        if session_id == None:
            ...
            self._session_id = session_id
        self._data[self._session_id] = dict(
            self._mongo.find_one({'session_id': self._session_id}))
    ...
    def renew(self, session_id):
        if not session_id:
            ...
            self._session_id = session_id
        self._data[self._session_id] = self._mongo.find_one(
    ...
    def save(self, expire=None):
        if expire:
            self._data[self._session_id]['expire'] = int(
                expire) + int(time.time())
        else:
            if int(self._expire) != 0:
                self._data[self._session_id]['expire'] = int(
                    time.time()) + int(expire)
        self._mongo.update_one({'session_id': self._session_id}, {
                               "$set": self._data[self._session_id]}, upsert=True)

    def delete(self, session_id=None):
        if session_id:
            if session_id in self._data:
                del self._data[session_id]
            session = self._mongo.find_one_and_delete(
                {'session_id': session_id}, projection={'session_id': True})
            return session.get('session_id')
        else:
            del self._data[self._session_id]
            session = self._mongo.find_one_and_delete(
                {'session_id': self._session_id}, projection={'session_id': True})
            self._session_id = self._generate_session_id()
            self._data[self._session_id] = {}
            return session.get('session_id')
...
class RedisSession(AbstractSession):
    ...
    def __init__(self, session_id=None, config=None):
        ...
        if not type(self)._pool:
            type(self)._pool = redis.ConnectionPool(
        ...
        if not self._data.get(self._session_id):
            ...
            expire_delta_time = self._data[self._session_id].get(
                '__expire_delta_time'.encode('utf-8'), b'').decode('utf-8')
            if expire_delta_time:
    ...
    def save(self, expire=None):
        if expire:
            ...
            if int(self._expire) != 0:
                self._data[self._session_id][
                    '__expire_delta_time'] = self._expire
                self._redis.hmset(self._session_id, self._data[
                                  self._session_id])
...
class SessionManager(object):
    ...
    def __init__(self, session_id=None, *, config=None, driver=None):
        ...
        if not driver:
            ...
            self._default_driver = getattr(
                self, 'get_%ssession_driver' % driver)(config)
    ...
    def driver(self, driver_name, config=None):
        if driver_name in self._drivers:
            ...
            self._drivers[driver_name] = getattr(
                self, 'get_%ssession_driver' % driver_name)(config)
            self._specific_driver = self._drivers[driver_name]
        return self
