...
class Model(dict, metaclass=ModelMetaclass):
    ...
    def __init__(self, select=False, alias=False, **kw):
        ...
        if not select:
            for column_name in self.__columns__:
                columns[column_name] = self.__columns__[
    ...
    def findall(self):
        sql = 'select %s from `%s`' % (
            self.__fields__['fields'], self.__table__)
    ...
    def update(self, args):
        ...
        for k, v in args.items():
            m = re.match(
                r'^[a-zA-Z\w\s]*(char|test)[\(\)0-9a-zA-Z\w\s]*$', self.__columns__[k]['type'])
            if m:
    ...
    def save(self):
        ...
        if self.__fields__['values'].strip() == '':
            ...
            sql = sql + 'insert into `%s`(%s) values(%s)' % (
                self.__table__, ','.join(insertcolumns), ','.join(insertvalues))
        else:
            sql = sql + \
                'insert into `%s` %s ' % (
                    self.__table__, self.__fields__['fields'])
    ...
    def fields(self, field):
        if isinstance(field, list):
            ...
            self.__fields__['fields'] = ' '
            self.__fields__['fields'] = self.__fields__[
                'fields'] + ','.join(fields)
            self.__fields__['fields'] = self.__fields__['fields'] + ' '
            self.__fields__['values'] = ' values('
            self.__fields__['values'] = self.__fields__[
                'values'] + ','.join(values) + ') '
    ...
    def where(self, name, op, value):
        ...
        if not self.__query__['where'] == '':
            ...
            self.__query__['where'] = self.__query__['where'] + " `%s` %s %s " % (
                name, op, '"' + value + '"' if isinstance(value, str)else str(value))
        else:
            self.__query__['where'] = "where `%s` %s %s " % (
                name, op, '"' + value + '"' if isinstance(value, str)else str(value))
        return self
