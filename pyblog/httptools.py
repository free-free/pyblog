...
class AppContainer(dict):
    ...
    def clear_cookie(self, name, *, path='/', domain=None):
        self._app['del_cookie'].append(
            {'cookie_name': name, 'path': path, 'domain': domain})
        ...
        if not hasattr(self, '_session_instance'):
            ...
            if not session_id:
                self._session_instance = SessionManager(
                    driver=self._config.session.driver_name, config=self._config.session.all)
            else:
                self._session_instance = SessionManager(
                    session_id, driver=self._config.session.driver_name, config=self._config.session.all)
        return self._session_instance

    def session_end(self, expire=None, *, auth=False):
        if hasattr(self, '_session_instance'):
            if auth:
                m = hmac.new(str(self._session_instance[Config.authentication.auth_id]).encode(
                    "utf-8"), self._session_instance.session_id.encode("utf-8"), hashlib.sha1)
        ...
        if message:
            ...
            try:
                ...
                error_page = (DEFAULT_HTTP_ERROR_PAGE % (
                    code, "Pyblog 1.0", code, HTTP_ERROR_REASON_EN.get(code)))
...
class Middleware(object):
        ...
        def _handler(request):
            try:
                ...
                if Config.app.debug:
                    ...
                    error_page = DEFAULT_ERROR_PRINT_PAGE % (
                        error.replace('\n', '<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'))
                    logging.error(error)
                else:
                    try:
                        ...
                        error_page = app.get(
                            '__templating__').render(error_template)
                    except jinja2.exceptions.TemplateNotFound:
                        error_page = DEFAULT_HTTP_ERROR_PAGE % (
                ...
                if app.get('status'):
                    res = web.Response(status=app.get('status').get(
                        'code'), body=app.get('status').get('message').encode('utf-8'))
                    return res
            return res
        return _handler
        ...
        def _cookie(request):
            ...
            if app.get('set_cookie') and len(app['set_cookie']) > 0:
                for k in app['set_cookie']:
                    res.set_cookie(k['cookie_name'], k['cookie_value'], path=k['path'], expires=k[
                                   'expire'], domain=k['domain'], httponly=k['httponly'])
            if app.get('del_cookie') and len(app['del_cookie']) > 0:
                for k in app['del_cookie']:
                    res.del_cookie(k['cookie_name'], path=k[
                                   'path'], domain=k['domain'])
            return res
        return _cookie
    ...
    def response_middleware(app, handler):
        ...
        def _response(request):
            ...
            if isinstance(res, web.StreamResponse):
                ...
                if template is None:
                    res = web.Response(body=json.dumps(
                        res, ensure_ascii=False, default=lambda x: x.__dict__).encode("utf-8"))
                    res.content_type = 'application/json;charset=utf-8'
                else:
                    res = web.Response(body=app['__templating__'].render(
                        template, **res['parameter']).encode("utf-8"))
                    res.content_type = 'text/html;charset=utf-8'
            else:
                res = res
            return res
                ...
                if matched_handler.__auth__:
                    ...
                    if session_id:
                        session = SessionManager(
                            session_id, driver=Config.session.driver_name, config=Config.session.all)
                    else:
                        session = SessionManager(
                            driver=Config.session.driver_name, config=Config.session.all)
                    user_id = session[Config.authentication.auth_id]
                    if not user_id:
                        res = aiohttp.web.HTTPFound(
                            Config.authentication.login_url)
                        return res
                    else:
                        unss = request.cookies.get(
                            'unss') or request.GET.get('unss')
                        if not unss:
    ...
    def allmiddlewares(cls):
        middlewares = list()
        raw_middlewares = [cls.auth_middleware, cls.log_middleware,
                           cls.http_error_middleware, cls.cookie_middleware, cls.response_middleware]
...
class Route(object):
    ...
    def process_routes(cls, app, routes):
        for handler in routes:
            ...
            if _path is None or _method is None:
                raise ValueError(
                    "__method__ or __url__ not define in %s." % str(handler.__name__))
            ...
            if len(_path) > 1 and _path.endswith('/'):
                app.router.add_route(_method, _path.rsplit(
                    '/', 1)[0], RequestHandler(app, handler))
                app.router.add_route(
                    _method, _path, RequestHandler(app, handler))
            elif not _path.endswith('/'):
                app.router.add_route(
                    _method, _path, RequestHandler(app, handler))
                app.router.add_route(_method, _path + '/',
    ...
    def register_route(cls, app):
        Route._none_variable_routes = cls.sort_none_variable_routes(
            Route._none_variable_routes)
        cls.process_routes(app, Route._none_variable_routes)
        Route._incomplete_variable_routes = cls.sort_variable_routes(
            Route._incomplete_variable_routes)
        cls.process_routes(app, Route._incomplete_variable_routes)
        Route._complete_variable_routes = cls.sort_variable_routes(
            Route._complete_variable_routes)
